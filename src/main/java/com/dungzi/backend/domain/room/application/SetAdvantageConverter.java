...
public class SetAdvantageConverter implements AttributeConverter<EnumSet<Advantage>, String>  {

    @Override
    public String convertToDatabaseColumn(EnumSet<Advantage> attribute) {
    ...
    @Override
    public EnumSet<Advantage> convertToEntityAttribute(String dbData) {
        // DB에서 읽어온 값이 null이거나 공백이거나 CATEGORY.KOREA(name="한식") 형태로 읽어올 경우 제외
        if(dbData == null || dbData == "" || dbData.contains(".")) return EnumSet.noneOf(Advantage.class);
        // 최초 빈 Collection 생성
        EnumSet<Advantage> attribute = EnumSet.noneOf(Advantage.class);
        ...
        Arrays.stream(dbDataArray).forEach(e -> attribute.add(Advantage.valueOf(e)));
        return attribute;
    }
}
