...
public class SetOptionConverter implements AttributeConverter<EnumSet<Options>, String> {

    // DB에서 사용할 Type으로 Entity Type을 변환하는 로직 구현
    @Override
    public String convertToDatabaseColumn(EnumSet<Options> attribute) {
    ...
    @Override
    public EnumSet<Options> convertToEntityAttribute(String dbData) {
        // DB에서 읽어온 값이 null이거나 공백이거나 CATEGORY.KOREA(name="한식") 형태로 읽어올 경우 제외
        if(dbData == null || dbData == "" || dbData.contains(".")) return EnumSet.noneOf(Options.class);
        // 최초 빈 Collection 생성
        EnumSet<Options> attribute = EnumSet.noneOf(Options.class);
        ...
        Arrays.stream(dbDataArray).forEach(e -> attribute.add(Options.valueOf(e)));
        return attribute;
    }
}